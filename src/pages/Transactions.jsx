...
import { Plus, Search, Filter, Trash2, Pencil, X, Check } from 'lucide-react'
...
function TransactionModal({ onClose, existing }) {
  ...
  const handleSubmit = (e) => {
    e.preventDefault()
    if (!form.name.trim())  return setError('Veuillez saisir un libellé.')
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
            ...
            {types.map(t => (
              <button
                ...
                className={`flex-1 py-2 text-xs font-medium rounded-lg border transition-all
                  ${form.type === t.value
                    ? t.active
                    : 'bg-gray-50 text-gray-500 border-gray-200 dark:border-gray-800 hover:bg-gray-100'
                  }`}
              >
                {t.label}
              </button>
          ...
          <div>
            ...
            <input
              ...
              placeholder="0"
              min="0"
              className="w-full px-3 py-2.5 border border-gray-200 dark:border-gray-800 rounded-lg text-sm focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100 transition-all"
            />
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input
                ...
                value={form.date}
                onChange={handleChange}
                className="w-full px-3 py-2.5 border border-gray-200 dark:border-gray-800 rounded-lg text-sm focus:outline-none focus:border-blue-500"
              />
            </div>
            <div>
...
function TxRow({ tx, onEdit, onDelete }) {
  ...
  return (
    <div className="flex items-center gap-3 py-3 border-b border-gray-50 last:border-0 group">
      ...
      {tx.note && (
        <p className="text-xs text-gray-400 dark:text-gray-500 italic hidden md:block max-w-[120px] truncate">
          {tx.note}
        </p>
      )}
    ...
    <div className="p-6 max-w-4xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-400 dark:text-gray-500 text-sm mt-0.5">{filtered.length} transaction(s) trouvée(s)</p>
        </div>
        <button
      ...
      <div className="relative mb-4">
        ...
        <input
          ...
          className="w-full pl-9 pr-4 py-2.5 border border-gray-200 dark:border-gray-800 rounded-lg text-sm focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100 bg-white dark:bg-gray-900"
        />
      </div>
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-full text-xs font-medium border transition-all
              ${filter === f.key
                ...
                : 'bg-white dark:bg-gray-900 text-gray-500 border-gray-200 dark:border-gray-800 hover:border-blue-300 hover:text-blue-600'
              }`}
          >
            {f.label}
          </button>
      ...
      {filtered.length > 0 && (
        <div className={`mb-4 px-4 py-2.5 rounded-lg text-sm font-medium flex items-center justify-between
          ${totalFiltered >= 0 ? 'bg-emerald-50 text-emerald-700' : 'bg-red-50 text-red-600'}`}>
      ...
      {sortedDates.length === 0 ? (
        <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-12 text-center">
          <p className="text-gray-400 dark:text-gray-500 text-sm">Aucune transaction trouvée.</p>
          <button
            onClick={() => setShowModal(true)}
