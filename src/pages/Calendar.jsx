        ...
        <div className="lg:col-span-2 bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-4">
          ...
          <div className="grid grid-cols-7 gap-1">
            {Array.from({ length: totalCells }).map((_, i) => {
              ...
              let key = null
              if (isPrev) {
                displayDay = daysInPrevMonth - offset + i + 1
              } else if (isNext) {
              ...
              return (
                <button
                  ...
                  className={`
                    ...
                    pt-1.5 pb-1 px-1 rounded-lg transition-all text-xs
                    ${isOther
                    ...
                    <div className="flex gap-0.5 mt-1">
                      {hasIncome && (
                        <span className={`w-1.5 h-1.5 rounded-full
                          ${selCell ? 'bg-white' : 'bg-emerald-400'}`}
                        />
                      )}
                      {hasExpense && (
                        <span className={`w-1.5 h-1.5 rounded-full
                          ${selCell ? 'bg-white/70' : 'bg-red-400'}`}
                        />
                      )}
                    </div>
                  )}

                  {/* Point "aujourd'hui" dans le cercle sélectionné */}
                  {todayCell && !selCell && (
          ...
          <div className="flex gap-4 mt-4 pt-3 border-t border-gray-100 dark:border-gray-800">
            <div className="flex items-center gap-1.5 text-xs text-gray-400">
              <span className="w-2 h-2 rounded-full bg-emerald-400" /> Revenu
            </div>
            <div className="flex items-center gap-1.5 text-xs text-gray-400">
              <span className="w-2 h-2 rounded-full bg-red-400" /> Dépense
            </div>
            <div className="flex items-center gap-1.5 text-xs text-gray-400">
              <span className="w-2 h-2 rounded-full bg-blue-600" /> Aujourd'hui
            </div>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-4 flex flex-col">
          ...
          {selectedTx.length === 0 ? (
            <div className="flex-1 flex flex-col items-center justify-center text-center py-8">
              <div className="w-12 h-12 bg-gray-100 dark:bg-gray-800 rounded-xl flex items-center justify-center text-2xl mb-3">
                📅
              </div>
              ...
              <div className="flex-1 overflow-y-auto space-y-2">
                {selectedTx
                  ...
                  .map(tx => (
                    <div
                      key={tx.id}
                      className="flex items-center gap-2.5 p-2.5 bg-gray-50 dark:bg-gray-800 rounded-lg"
                    >
