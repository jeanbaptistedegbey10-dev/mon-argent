...
function BudgetModal({ onClose, existing }) {
  const { categories, budgets } = useFinanceStore()
  ...
  const handleSubmit = (e) => {
    ...
    if (existing) {
      store.updateBudget(existing.id, {
                ...
                <button
                  ...
                  className={`flex items-center gap-2 px-3 py-2 rounded-lg border text-xs font-medium transition-all
                    ${form.name === cat.name
                      ...
                      : 'border-gray-200 dark:border-gray-800 text-gray-600 hover:border-gray-300 hover:bg-gray-50'
                    }`}
                >
                  <span>{cat.icon}</span>
          ...
          {form.name && form.limit && (
            <div className="p-3 bg-gray-50 rounded-lg">
              <div className="flex items-center justify-between text-xs mb-2">
                ...
                <span className="text-gray-400 dark:text-gray-500">0 / {fmt(Number(form.limit))} FCFA</span>
              </div>
    ...
    <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-5 hover:border-gray-300 transition-colors">
      ...
      <div className="flex items-start justify-between mb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-sm font-semibold text-gray-800">{budget.name}</p>
            <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">Budget mensuel</p>
          </div>
        </div>
      ...
      <div className="flex items-end justify-between mb-2">
        <div>
          <span className="text-xl font-bold text-gray-900 dark:text-white">{fmt(budget.spent)}</span>
          <span className="text-sm text-gray-400 dark:text-gray-500 ml-1">/ {fmt(budget.limit)} FCFA</span>
        </div>
      ...
      <div className={`flex items-center gap-1.5 text-xs font-medium px-3 py-1.5 rounded-lg ${statusBg}`}>
        {over ? (
          <>
            ...
            Dépassé de {fmt(budget.spent - budget.limit)} FCFA
          </>
        ) : warning ? (
          <>
            <AlertTriangle size={12} />
            Attention — plus que {fmt(remain)} FCFA restants
          </>
        ) : (
          <>
            <TrendingUp size={12} />
            {fmt(remain)} FCFA restants ce mois
          </>
        )}
      </div>
    </div>
    ...
    <div className="p-6 max-w-4xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-400 dark:text-gray-500 text-sm mt-0.5">Suivez vos limites de dépenses</p>
        </div>
        <button
      ...
      <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-5 mb-6">
        <div className="flex items-center justify-between mb-3">
          <div>
            <p className="text-xs text-gray-400 dark:text-gray-500 font-medium">Budget global du mois</p>
      ...
      {budgets.length === 0 ? (
        <div className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-12 text-center">
          <p className="text-gray-400 dark:text-gray-500 text-sm mb-3">Aucun budget configuré.</p>
          <button
            onClick={() => setShowModal(true)}
      ...
      {showModal && (
        <BudgetModal onClose={handleClose} existing={editTarget} />
      )}
    </div>
  )
}